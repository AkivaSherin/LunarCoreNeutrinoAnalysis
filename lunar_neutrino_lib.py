...
def make_muon_detections_pdf(core_mantle_density_ratio, core_radius):
    ...
    for log_energy in log_energies:
        ...
        actual_muons_detected = neutrinos_detected_ignoring_oscillation * \
                                monte_carlo_uniform_core(10 ** log_energy, core_mantle_density_ratio, core_radius)[
                                    1]  # factors in expected flavor ratio
    ...
    pdf_interp = interp1d(log_energies, pdf(log_energies),
                          kind='linear')  # we have to re-interpolate so that we can save our spline
...
def muon_pdf_spline(find_core_mantle_density_ratio, find_core_radius):
    ...
    for i in range(0, index_val):
        ...
        c1 = (np.round(muon_pdf_splines_catalog['core_mantle_density_ratio'][i], 2) == np.round(
            find_core_mantle_density_ratio, 2))
    ...
    if found == False:
        # generate new spline...
        print(r'Generating new energy pdf spline: core-mantle density ratio =',
              np.round(find_core_mantle_density_ratio, 2), r'core radius =', np.round(find_core_radius, 0))
...
def make_mock_data(core_mantle_density_ratio, core_radius, num_neutrinos):
    pdf = muon_pdf_spline(core_mantle_density_ratio,
                          core_radius)  # make_muon_detections_pdf(core_mantle_density_ratio, core_radius)
...
def find_test_statistic(core_mantle_density_ratio_test_hypothesis, core_radius_test_hypothesis, data):
    test_hypothesis_log_likelihood = find_log_likelihood(core_mantle_density_ratio_test_hypothesis,
                                                         core_radius_test_hypothesis, data)
...
def find_best_fit_parameters(data):
    core_mantle_density_ratios = np.linspace(1, 15, 100) #eventually change to 100
...
def make_mock_data_sets(density_ratio, radius, num_mock_data_sets, neutrinos_per_set):
    ...
    for i in range(num_mock_data_sets):
        data_sets.append(make_mock_data(density_ratio, radius,
                                        neutrinos_per_set))  # our null hypothesis is 1.77 density ratio, 330 km core radius
