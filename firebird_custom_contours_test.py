import importlib
lunar_neutrino_lib = importlib.import_module("lunar_neutrino_lib")
importlib.reload(lunar_neutrino_lib)
from lunar_neutrino_lib import *

ratios = [3, 7, 30]
radii = [100, 250, 400]
for ratio in ratios:
    for radius in radii:
        make_heatmap_confidence_interval_plot(ratio, radius, 1000, percentiles=(30, 50, 90))
        print("made a heatmap 1000 neutrinos")

for ratio in ratios:
    for radius in radii:
        make_heatmap_confidence_interval_plot(ratio, radius, 1000, percentiles=(20, 60, 95))
        print("made a heatmap 1000 neutrinos")